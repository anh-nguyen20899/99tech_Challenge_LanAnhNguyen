...
function App() {
  ...
  const [amountForReceive, setAmountForReceive] = useState(0);
  const handleChange = (event) => {
    const { name, value, selectedIndex } = event.target;
    const selectedOption = currencyExchange[selectedIndex];
  ...
  const handleSubmit = (event) => {
    ...
    if (Object.keys(validationErrors).length === 0) {
      ...
      const result = parseInt(formData.amountForSend) * parseInt(formData.currency);
      console.log(result);
      setAmountForReceive(result);
    } else {
  ...
  const validateForm = (data) => {
    let errors = {};
    if (!data.amountForSend.trim() && data.amountForSend !== 'number') {
      errors.amountForSend = 'Amount To Send is required. It should be a number';
    }
    if (!data.currency) {
  ...
  return (
    <form onSubmit={handleSubmit}>
      ...
      <input id="input-amount" 
          type="text"
          name="amountForSend"
          value={formData.amountForSend}
          onChange={handleChange}/>
          {errors.amountForSend && <span>{errors.amountForSend}</span>}
      ...
      <label>
          ...
          <select name="currency" value={formData.currency} onChange={handleChange}>
            ...
            {currencyExchange.map((option, index) => (
              <option key={index} value={option.price}>{option.currency}</option>
            ))}
          </select>
      ...
      <input id="output-amount" 
        type="text"
        name="amountForReceive"
        value={amountForReceive}/>
